...
class AppRouter {
  ...
  /// Builds the customer-import wizard screen (TASK-167), given
  /// `orgId`/`companyId` from [CustomerImportRoute].
  final Widget Function(BuildContext context, String orgId, String companyId)?
  ...
  /// Builds the Sales Dashboard screen (TASK-135), given `orgId`/`companyId`
  /// and the raw `queryParameters` of [SalesDashboardRoute] — the caller
  /// decides how to turn those into a `SalesDashboardFilters`, same "router
  /// hands raw params, page owns parsing" contract
  /// [executiveDashboardPageBuilder] already sets.
  final Widget Function(
    BuildContext context,
    String orgId,
    String companyId,
  ...
  /// Builds the Collection Dashboard screen (TASK-138), given `orgId`/
  /// `companyId` and the raw `queryParameters` of [CollectionDashboardRoute]
  /// — the caller decides how to turn those into a
  /// `CollectionDashboardFilters`, same "router hands raw params, page owns
  /// parsing" contract [productDashboardPageBuilder] already sets.
  final Widget Function(
    BuildContext context,
    String orgId,
    String companyId,
  ...
  late final GoRouter router = GoRouter(
    ...
    routes: [
      ...
      GoRoute(
        path: SalesDashboardRoute.pathPattern,
        name: SalesDashboardRoute.name,
        redirect: (context, state) => authorizationGuard.redirect(
          context,
          state,
          requiredCapability: Capability.reportViewSensitive,
      ...
      GoRoute(
        path: CollectionDashboardRoute.pathPattern,
        name: CollectionDashboardRoute.name,
        redirect: (context, state) => authorizationGuard.redirect(
          context,
          state,
          requiredCapability: Capability.reportViewSensitive,
      ...
      GoRoute(
        path: CustomerImportRoute.pathPattern,
        name: CustomerImportRoute.name,
        redirect: (context, state) => authorizationGuard.redirect(
          context,
          state,
          requiredCapability: Capability.customerImport,
        ),
        builder: (context, state) {
      ...
      GoRoute(
        path: CartSharePublicRoute.pathPattern,
