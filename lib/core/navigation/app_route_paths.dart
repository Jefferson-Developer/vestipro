...
/// Product-import wizard route (TASK-168, EPIC-22), scoped by Organization
/// and Company. Protected in [AppRouter] by `product.import`
/// (OWNER/ADMIN only, see `RolePermissionMatrix`).
final class ProductImportRoute extends AppRoute {
  const ProductImportRoute({required this.orgId, required this.companyId});

  final String orgId;
  final String companyId;

  static const name = 'productImport';
  static const pathPattern = '/org/:orgId/companies/:companyId/products/import';

  @override
  String get location => '/org/$orgId/companies/$companyId/products/import';
}
...
/// Central de Oportunidades route (TASK-132, EPIC-16), scoped by
/// Organization and Company. [queryParameters] carries the active
/// `OpportunityCenterFilters` (see
/// `OpportunityCenterFilters.toQueryParameters`), so a Flutter Web
/// reload/shared link restores exactly the same filter/sort state, same
/// precedent [OrderListRoute]/[CustomerPortfolioRoute] already set.
/// Protected in [AppRouter] by `insight.view`.
final class OpportunityCenterRoute extends AppRoute {
  const OpportunityCenterRoute({
    required this.orgId,
    required this.companyId,
    this.queryParameters = const <String, String>{},
  });
  ...
  static const name = 'opportunityCenter';
  static const pathPattern =
      '/org/:orgId/companies/:companyId/insights/opportunities';

  @override
  String get location {
    final path = '/org/$orgId/companies/$companyId/insights/opportunities';
    if (queryParameters.isEmpty) return path;
    return Uri(path: path, queryParameters: queryParameters).toString();
  }
}
...
/// Sales Dashboard route (TASK-135, EPIC-17), scoped by Organization and
/// Company. [queryParameters] carries `teamId`/`month`/`groupBy`/`compare`/
/// `sortBy`/`sortDir` (see `SalesDashboardFilters.toQueryParameters`), so a
/// Flutter Web reload/share link restores exactly the same view, same
/// precedent [ExecutiveDashboardRoute] already sets. Protected in
/// [AppRouter] by `report.viewSensitive` — same capability
/// [ExecutiveDashboardRoute] itself requires, since both dashboards read the
/// same five TASK-133 aggregation collections.
final class SalesDashboardRoute extends AppRoute {
  ...
  static const name = 'salesDashboard';
  static const pathPattern =
      '/org/:orgId/companies/:companyId/dashboards/sales';

  @override
  String get location {
    final path = '/org/$orgId/companies/$companyId/dashboards/sales';
    if (queryParameters.isEmpty) return path;
    return Uri(path: path, queryParameters: queryParameters).toString();
  }
}
