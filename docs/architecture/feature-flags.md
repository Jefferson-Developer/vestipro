...
| `config_report_export_max_local_rows` | integer | `5000` | Maximum result row count the report builder still exports entirely on-device — CSV (TASK-146) or XLSX (TASK-147) — above it, export is delegated to the matching `exportReportToCsv`/`exportReportToXlsx` Cloud Function, which re-runs the aggregation server-side and uploads the file to Storage. A UX/performance knob only, never a security boundary. | `flutter-senior-architect` | 2026-09-04 | 2027-03-04 |
