  ...
  group('DriftFavoriteRepository', () {
    ...
    test(
      ...
      () async {
        ...
        expect(
          (await _favoritesPage(repository, userId: 'user-1')).items,
          isEmpty,
        );
    ...
    test(
      'listFavorites returns an empty page when nothing is favorited',
      () async {
        final page = await _favoritesPage(repository, userId: 'user-1');
    ...
    test(
      'listFavorites paginates without losing or duplicating a product',
      () async {
    ...
    test('favoriting while the remote push fails ("offline") still favorites '
        'locally right away, independent of the remote call', () async {
    ...
    test('a favorite created offline syncs to the remote store once it stops '
        'failing (simulating reconnection)', () async {
      remote.failUpserts = true;
      await repository.addFavorite(
        organizationId: 'org-1',
        userId: 'user-1',
    ...
    test('an unfavorite made while the remote delete fails keeps the product '
        'out of listFavorites locally (never resurrects it)', () async {
      await repository.addFavorite(
        organizationId: 'org-1',
        userId: 'user-1',
        productId: 'product-1',
    ...
    test(
      'favorites never leak between organizations (multi-tenant isolation)',
      () async {
        await repository.addFavorite(
          organizationId: 'org-1',
          userId: 'user-1',
    ...
    test(
      'favorites never leak between users of the same organization',
      () async {
        await repository.addFavorite(
          organizationId: 'org-1',
          userId: 'user-1',
