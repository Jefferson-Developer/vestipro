...
void main() {
  group('QualifyLeadUseCase', () {
    late _MockLeadRepository repository;
    late QualifyLeadUseCase useCase;
    ...
    setUp(() {
      ...
      useCase = QualifyLeadUseCase(repository);
    });

    test('qualifies a contacted lead', () async {
      ...
      expect(updated.qualifiedAt, isNotNull);
      expect(updated.version, lead.version + 1);
    });

    test(
      'blocks qualifying a brand new lead that was never contacted',
      () async {
    ...
    test('blocks qualifying an already disqualified lead', () async {
      final lead = _buildLead(status: LeadStatus.disqualified);
      when(
        () => repository.getById(
          organizationId: any(named: 'organizationId'),
          id: any(named: 'id'),
