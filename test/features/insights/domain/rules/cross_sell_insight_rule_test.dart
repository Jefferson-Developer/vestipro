...
void main() {
  group('CrossSellInsightRule', () {
    ...
    test('fires when a category popular among similar customers is absent from '
      ...
      expect(
        insight.evidence
            .firstWhere((item) => item.code == 'similarity_group')
            .value,
    ...
    test('does not fire when the customer already buys every relevant category '
    ...
    test('limits suggestions to 3 per customer, ordered by relevance '
      ...
      expect(
        insight.evidence
            .where(
              (item) => item.code.startsWith('cross_sell_category_adoption:'),
            )
            .length,
        3,
      );
    ...
    test('excludes a category unavailable in the customer price list even when '
