...
void main() {
  ...
  blocTest<ReportBuilderBloc, ReportBuilderState>(
    'PDF preview requested below the configured threshold generates bytes without saving anything (TASK-148)',
    build: buildBloc,
    act: (bloc) async {
      bloc.add(
        const ReportBuilderStarted(
      ...
      bloc.add(const ReportExecutionRequested());
      await Future<void>.delayed(const Duration(milliseconds: 20));
      bloc.add(const ReportPdfPreviewRequested());
    },
    wait: const Duration(milliseconds: 20),
    verify: (bloc) {
      expect(exportRepository.pdfCloudRequests, isEmpty);
      expect(exportRepository.savedFileNames, isEmpty);
      expect(exportRepository.encodePdfCalls, 1);
      expect(exportRepository.brandingLookups, <String>['org-a']);
      expect(bloc.state.pdfPreviewStatus, ReportPdfPreviewStatus.ready);
      expect(bloc.state.pdfPreview, isNotNull);
      expect(bloc.state.pdfPreview!.fileName, endsWith('.pdf'));
      expect(bloc.state.exportStatus, ReportExportStatus.idle);
    },
  );

  blocTest<ReportBuilderBloc, ReportBuilderState>(
    'confirming a PDF preview saves the already-generated bytes without re-encoding (TASK-148)',
    build: buildBloc,
    act: (bloc) async {
      bloc.add(
        const ReportBuilderStarted(
      ...
      bloc.add(const ReportPdfPreviewRequested());
      await Future<void>.delayed(const Duration(milliseconds: 20));
      bloc.add(const ReportPdfExportConfirmed());
    },
    wait: const Duration(milliseconds: 20),
    verify: (bloc) {
      expect(exportRepository.encodePdfCalls, 1);
      expect(exportRepository.savedFileNames, hasLength(1));
      expect(exportRepository.savedFileNames.single, endsWith('.pdf'));
      expect(bloc.state.exportStatus, ReportExportStatus.success);
      expect(bloc.state.exportSummary?.isRemote, isFalse);
      expect(bloc.state.pdfPreviewStatus, ReportPdfPreviewStatus.idle);
      expect(bloc.state.pdfPreview, isNull);
      final exportedEvent = analytics.loggedEvents.firstWhere(
        (event) => event.name == AnalyticsEvents.reportExported,
      );
      expect(exportedEvent.parameters?['formato'], 'pdf');
      expect(exportedEvent.parameters?['delegated_to_cloud'], false);
    },
  );

  blocTest<ReportBuilderBloc, ReportBuilderState>(
    'cancelling a PDF preview discards it without saving anything (TASK-148)',
    build: buildBloc,
    act: (bloc) async {
      bloc.add(
        const ReportBuilderStarted(
      ...
      bloc.add(const ReportPdfPreviewRequested());
      await Future<void>.delayed(const Duration(milliseconds: 20));
      bloc.add(const ReportPdfPreviewCancelled());
    },
    wait: const Duration(milliseconds: 20),
    verify: (bloc) {
      expect(exportRepository.savedFileNames, isEmpty);
  ...
  blocTest<ReportBuilderBloc, ReportBuilderState>(
    'PDF export above the configured threshold delegates to the Cloud Function without a local preview (TASK-148)',
    build: buildBloc,
    setUp: () => featureFlags.overrideFlag(
      FeatureFlagRegistry.configReportExportMaxLocalRows,
      0,
    ...
    wait: const Duration(milliseconds: 20),
    verify: (bloc) {
      expect(exportRepository.encodePdfCalls, 0);
      expect(exportRepository.pdfCloudRequests, hasLength(1));
      expect(bloc.state.pdfPreviewStatus, ReportPdfPreviewStatus.idle);
      expect(bloc.state.exportStatus, ReportExportStatus.success);
      expect(bloc.state.exportSummary?.isRemote, isTrue);
    },
  ...
  blocTest<ReportBuilderBloc, ReportBuilderState>(
    'a PDF preview generation failure surfaces without touching exportStatus (TASK-148)',
    build: buildBloc,
    setUp: () => exportRepository.throwOnEncodePdf = true,
    act: (bloc) async {
      bloc.add(
        const ReportBuilderStarted(
          organizationId: 'org-a',
    ...
    wait: const Duration(milliseconds: 20),
    verify: (bloc) {
      expect(bloc.state.pdfPreviewStatus, ReportPdfPreviewStatus.failure);
      expect(bloc.state.pdfPreviewFailure, isNotNull);
      expect(bloc.state.exportStatus, ReportExportStatus.idle);
    },
  );
}
...
final class _ReportExportRepository implements ReportExportRepository {
  ...
  final List<ReportDefinition> pdfCloudRequests = <ReportDefinition>[];
  final List<String> savedFileNames = <String>[];
  final List<String> brandingLookups = <String>[];
  int encodePdfCalls = 0;
  bool throwOnEncodePdf = false;
  AppResult<String> Function()? saveLocalOverride;
  AppResult<ReportExportSummary> Function()? cloudOverride;
  ReportBranding brandingOverride = const ReportBranding.none();
  ...
  Future<List<int>> encodePdf({
    ...
    encodePdfCalls++;
    if (throwOnEncodePdf) throw StateError('encoding failed');
    return const <int>[1, 2, 3];
  }

  @override
  Future<ReportBranding> loadBranding(String organizationId) async {
    brandingLookups.add(organizationId);
  ...
  Future<AppResult<ReportExportSummary>> requestCloudPdfExport({
    ...
    pdfCloudRequests.add(definition);
    final override = cloudOverride;
    if (override != null) return override();
    return AppSuccess<ReportExportSummary>(
      ReportExportSummary(
        fileName: 'remote_report.pdf',
        rowCount: 999,
        location: RemoteReportExportLocation(
          downloadUrl: 'https://example.com/remote_report.pdf',
          expiresAt: DateTime.utc(2026, 9, 5),
        ),
      ),
    );
